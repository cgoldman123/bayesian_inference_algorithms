...
function simulate_sampling_inf(num_neurons::Integer,
                             W::AbstractMatrix,
                             num_samples::Integer,
    ...
    for t in 1:num_samples-1
        for i in 1:num_neurons
            if !isnan(clamp[i])
                ...
                activation[i,t] = rand() < activation_prob
            end
        end
...
activation = simulate_sampling_inf(num_neurons, W, num_samples, b, clamp; rng = MersenneTwister(13))
...
println("Boltzman Gibbs' Sampling\nmu1: $mu1 \nmu2: $mu2")
